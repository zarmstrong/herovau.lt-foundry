...
let element=html.find(".window-header .window-title");if(element.length!=1)return;let vaultButton=$(`<a class="popout"><i class="fas fa-cloud"></i>Vault</a>`);vaultButton.on('click',()=>checkNextAction(obj.object));element.after(vaultButton);if(game.modules.get('pathbuilder2e-import')?.active&&enablePB){$("a:contains('Import from Pathbuilder')").remove();}});function setHLOToken(){HLOuserToken=Cookie.get('herovault_user_token');}
...
console.log("%cHeroVau.lt/Foundry Bridge | %c"+JSON.stringify(responseJSON),hvColor1,hvColor4);if(responseJSON.status==1){hvUserToken=ut;game.settings.set('herovaultfoundry','skipTokenPrompt',true);skipTokenPrompt=true;}else{hvUserToken="";Cookie.set('herovault_user_token',"",-1);game.settings.set('herovaultfoundry','skipTokenPrompt',false);skipTokenPrompt=false;}}};if(hvDebug)
...
async function exportToHV(targetActor){try{let exportNewPC=false;let exportOverwritePC=false;let vaultInfo=false;let canOverwrite=false;hvUserToken=Cookie.get('herovault_user_token');let portrait,hvUID;if(targetActor.data.img.includes("mystery-man")==-1)
...
bdy=bdy+`<div><p><img src="${portrait}"><br>Please choose an action to perform:</p><div><hr/>`;new Dialog({title:"Import to your HeroVau.lt",content:bdy,buttons:menuButtons,default:"exportNew",close:async(html)=>{if(exportNewPC){hvUID="";let exportStatus=await exportPCtoHV(targetActor,hvUserToken,hvUID,true);if(exportStatus.error==true){ui.notifications.error("Error exporting: "+exportStatus.message);}else{ui.notifications.info(exportStatus.message);}}else if(exportOverwritePC){if(hvDebug)
console.log("export overwrite PC");let exportStatus=await exportPCtoHV(targetActor,hvUserToken,hvUID,false);if(exportStatus.error==true){ui.notifications.error("Error exporting: "+exportStatus.message);}else{ui.notifications.info(exportStatus.message);}}}}).render(true);}}catch(e){console.log(e);}}
...
xmlhttp.open("GET",heroVaultURL+"/foundrymodule.php?action=checkCharacter&userToken="+hvUserToken+"&charUID="+hvUID,true);xmlhttp.send();});};const getVaultSlots=(hvUserToken)=>{return new Promise((resolve)=>{let error=false;var xmlhttp=new XMLHttpRequest();xmlhttp.onreadystatechange=function(){if(this.readyState==4&&this.status==200){let responseJSON=JSON.parse(this.responseText);if(hvDebug)
...
request.send();var request2=new XMLHttpRequest();request2.open('GET',charImport.img,true);request2.onreadystatechange=function(){if(this.status===404){console.log("got a 404");targetActor.update({'img':"icons/svg/mystery-man.svg"});}}
...
{domainParts=host.split('.');domainParts.shift();domain='.'+domainParts.join('.');document.cookie=name+"="+value+expires+"; path=/; domain="+domain+"; SameSite=Strict";if(Cookie.get(name)==null||Cookie.get(name)!=value)
{domain='.'+host;document.cookie=name+"="+value+expires+"; path=/; domain="+domain+"; SameSite=Strict";}}},get:function(name)
{var nameEQ=name+"=";var ca=document.cookie.split(';');for(var i=0;i<ca.length;i++)
{var c=ca[i];while(c.charAt(0)==' ')
{c=c.substring(1,c.length);}
if(c.indexOf(nameEQ)==0)return c.substring(nameEQ.length,c.length);}
return null;},erase:function(name)
{Cookie.set(name,'',-1);}};let HVversionCompare=function(left,right){if(typeof left+typeof right!='stringstring')
