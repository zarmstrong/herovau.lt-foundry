...
if(game.modules.get('pathbuilder2e-import')?.active&&enablePB){$("a:contains('Import from Pathbuilder')").remove();}});function setHLOToken(){HLOuserToken=Cookie.get('herovault_user_token');}
function checkUserToken(){var ut=Cookie.get('herovault_user_token');var xmlhttp=new XMLHttpRequest();xmlhttp.onreadystatechange=function(){if(this.readyState==4&&this.status==200){let responseJSON=JSON.parse(this.responseText);if(hvDebug)
...
async function loadPB(obj){let{beginPathbuilderImport}=await import('/modules/pathbuilder2e-import/pathbuilder-import-min.js');beginPathbuilderImport(obj,true)}
async function loadHLO(obj){import('../hlo-importer/hlo-importer.min.js').then(hlomodule=>{let hlo=new hlomodule.HeroLabImporter;hlo.hloShim(obj);}).catch(err=>{console.log("error importing hlo: "+err.message)});}
