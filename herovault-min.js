...
console.log("%cHeroVau.lt/Foundry Bridge | %c"+JSON.stringify(responseJSON),hvColor1,hvColor4);if(responseJSON.status==1){hvUserToken=token;game.settings.set('herovaultfoundry','userToken',token);game.settings.set('herovaultfoundry','skipTokenPrompt',true);skipTokenPrompt=true;return true;}else{hvUserToken="";game.settings.set('herovaultfoundry','userToken',null);game.settings.set('herovaultfoundry','skipTokenPrompt',false);skipTokenPrompt=false;return false;}}};if(hvDebug)
...
async function exportToHV(targetActor){try{var hvUserToken=Cookie.get('hvut');var xmlhttp=new XMLHttpRequest();xmlhttp.onreadystatechange=function(){if(this.readyState==4&&this.status==200){let responseJSON=JSON.parse(this.responseText);if(hvDebug)
console.log("%cHeroVau.lt/Foundry Bridge | %c"+JSON.stringify(responseJSON),hvColor1,hvColor4);if(responseJSON.status==1){performExportToHV(targetActor);}else{hvUserToken="";Cookie.set('hvut',"",-1);ui.notifications.warn("Unable to load vault.  Please double-check your User Token.");game.settings.set('herovaultfoundry','skipTokenPrompt',false);getVaultToken(exportToHV,targetActor,hvUserToken);}}};if(hvDebug)
...
if(hasProperty(targetActor,"data.flags.herovault.uid")){hvUID=targetActor.data.flags.herovault.uid;let accChk=checkForAccess(hvUserToken,hvUID);canOverwrite=accChk;}
...
console.log("%cHeroVau.lt/Foundry Bridge | %ccan access?: "+canOverwrite,hvColor1,hvColor4);if(freeSlots<1&&canOverwrite==false){bdy=`<div><p>Unfortunately you do not have enough open slots in your <a href="https://herovau.lt">HeroVau.lt</a> to import this PC.<br>Please upgrade your account or delete a PC from your account to free up some space.</p><div><hr/>`;new Dialog({title:"Import to your HeroVau.lt",content:bdy,buttons:{yes:{icon:"<i class='fas fa-check'></i>",label:`Ok`}},default:"yes"}).render(true);}else{if(freeSlots>0){menuButtons={...menuButtons,exportNew:{icon:"<i class='fas fa-file-export'></i>",label:`Export to HeroVau.lt as New PC`,callback:()=>exportNewPC=true}}
