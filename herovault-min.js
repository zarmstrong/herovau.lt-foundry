let hvDebug=false;const hvVer="0.6.2";const heroVaultURL='https://herovau.lt';const hvColor1='color: #7bf542';const hvColor2='color: #d8eb34';const hvColor3='color: #ffffff';const hvColor4='color: #cccccc';const hvColor5='color: #ff0000';let HLOuserToken,hvUserToken,skipTokenPrompt;let enableHLO=true;let enablePB=true;let pfsEnabled=true;Hooks.on('ready',async function(){console.log("%cHeroVau.lt/Foundry Bridge | %cinitializing",hvColor1,hvColor4);if(Cookie.get('hvut')){game.settings.set('herovaultfoundry','userToken',Cookie.get('hvut'));hvUserToken=game.settings.get('herovaultfoundry','userToken')
Cookie.set('hvut',"",-1);}
if(Cookie.get('herovault_skiptoken')){skipTokenPrompt=Cookie.get('herovault_skiptoken');game.settings.set('herovaultfoundry','userToken',skipTokenPrompt);Cookie.set('herovault_skiptoken',"",-1);}
game.settings.register('herovaultfoundry','userToken',{name:"HeroVau.lt User Token",hint:"Please enter your personal user token from "+heroVaultURL+". Your HeroVau.lt token allows you to import and export PCs directly into your HeroVau.lt account.  This is not required to use the Pathbuilder or HeroLab Online features.",scope:'client',config:true,type:String,default:hvUserToken,});game.settings.register('herovaultfoundry','hlouserToken',{name:"HeroLab Online User Token (optional)",hint:"Please enter your personal user token. A user token allows external tools (like HeroVau.lt) to access the HLO server and perform export operations.",scope:'client',config:true,type:String,default:'',onChange:value=>(setHLOToken())});game.settings.register('herovaultfoundry','debugEnabled',{name:"Enable debug mode",hint:"Debug output will be written to the js console.",scope:'world',config:true,type:Boolean,default:false,onChange:value=>(hvDebug=game.settings.get('herovaultfoundry','debugEnabled'))});game.settings.register('herovaultfoundry','skipTokenPrompt',{name:"Skip Token Prompt",hint:"Once your HeroVau.lt user token is set, you will no longer be prompted to set it. Unchecking this makes HeroVau.lt prompt you for the User Token again.",scope:'world',config:true,type:Boolean,default:false,onChange:value=>(skipTokenPrompt=game.settings.get('herovaultfoundry','skipTokenPrompt'))});hvDebug=game.settings.get('herovaultfoundry','debugEnabled');HLOuserToken=game.settings.get('herovaultfoundry','hlouserToken');hvUserToken=game.settings.get('herovaultfoundry','userToken');skipTokenPrompt=game.settings.get('herovaultfoundry','skipTokenPrompt');});Hooks.on('renderActorSheet',function(obj,html){const actor=obj.actor;const v8=HVversionCompare(game.data.version,'0.8.5');if(hvDebug){if(v8==1)
...
let element=html.find(".window-header .window-title");if(element.length!=1)return;let head=html.find(".window-header");let hvButton=head.find("#herovault");if(hvButton.length==0){let vaultButton=$(`<a class="popout" id="herovault"><i class="fas fa-cloud"></i>Vault</a>`);vaultButton.on('click',()=>checkNextAction(obj.object));element.after(vaultButton);}
if(game.modules.get('pathbuilder2e-import')?.active&&enablePB){$("a:contains('Import from Pathbuilder')").remove();}});function setHLOToken(){HLOuserToken=game.settings.get('herovaultfoundry','hlouserToken');}
function checkUserToken(token){var xmlhttp=new XMLHttpRequest();xmlhttp.onreadystatechange=function(){if(this.readyState==4&&this.status==200){let responseJSON=JSON.parse(this.responseText);if(hvDebug)
console.log("%cHeroVau.lt/Foundry Bridge | %ccheckUserToken: "+JSON.stringify(responseJSON),hvColor1,hvColor4);if(responseJSON.status==1){hvUserToken=token;game.settings.set('herovaultfoundry','userToken',token);game.settings.set('herovaultfoundry','skipTokenPrompt',true);skipTokenPrompt=true;return true;}else{hvUserToken="";game.settings.set('herovaultfoundry','userToken',null);game.settings.set('herovaultfoundry','skipTokenPrompt',false);skipTokenPrompt=false;return false;}}};if(hvDebug)
console.log("%cHeroVau.lt/Foundry Bridge | %c/foundrymodule.php?action=iv&userToken="+token,hvColor1,hvColor4);xmlhttp.open("GET",heroVaultURL+"/foundrymodule.php?action=iv&userToken="+token,true);xmlhttp.send();}
...
function pfsDialogue(obj){let pfsnumber,pfscharnumber,searchPFS;new Dialog({title:`Pathfinder Society Import`,content:`
        ...
        `,buttons:{yes:{icon:"<i class='fas fa-check'></i>",label:`Import`,callback:()=>searchPFS=true},no:{icon:"<i class='fas fa-times'></i>",label:`Cancel`},},default:"yes",close:html=>{if(searchPFS){pfsnumber=html.find('[id="pfsnumber"]')[0].value;pfscharnumber=html.find('[id="pfscharnumber"]')[0].value;if(hvDebug)
...
function findPFS(obj,pfsnumber,pfscharnumber){var hvUserToken=game.settings.get('herovaultfoundry','userToken');var xmlhttp=new XMLHttpRequest();xmlhttp.onreadystatechange=function(){if(this.readyState==4&&this.status==200){let responseJSON=JSON.parse(this.responseText);if(hvDebug)
...
hvUserToken="";new Dialog({title:`HeroVau.lt Import`,content:`
      ...
      </style>`,buttons:{yes:{icon:"<i class='fas fa-check'></i>",label:`Load Vault`,callback:()=>applyChanges=true},no:{icon:"<i class='fas fa-times'></i>",label:`Cancel`},},default:"yes",close:html=>{if(applyChanges){let userToken=html.find('[id="textBoxUserToken"]')[0].value;let skipToken=html.find('[id="skipToken"]')[0].checked;checkUserToken(userToken);hvUserToken=userToken;if(skipToken)
game.settings.set('herovaultfoundry','skipTokenPrompt',true);callback(callbackArg1,callbackArg2,callbackArg3,callbackArg4);}}}).render(true);}
async function exportToHV(targetActor){try{var hvUserToken=game.settings.get('herovaultfoundry','userToken');var xmlhttp=new XMLHttpRequest();xmlhttp.onreadystatechange=function(){if(this.readyState==4&&this.status==200){let responseJSON=JSON.parse(this.responseText);if(hvDebug)
console.log("%cHeroVau.lt/Foundry Bridge | %c"+JSON.stringify(responseJSON),hvColor1,hvColor4);if(responseJSON.status==1){performExportToHV(targetActor);}else{hvUserToken="";game.settings.set('herovaultfoundry','userToken',null);ui.notifications.warn("Unable to load vault.  Please double-check your User Token.");game.settings.set('herovaultfoundry','skipTokenPrompt',false);getVaultToken(exportToHV,targetActor,hvUserToken);}}};if(hvDebug)
console.log("%cHeroVau.lt/Foundry Bridge | %c/foundrymodule.php?action=iv&userToken="+hvUserToken,hvColor1,hvColor4);xmlhttp.open("GET",heroVaultURL+"/foundrymodule.php?action=iv&userToken="+hvUserToken,true);xmlhttp.send();}catch(e){console.log(e);}}
async function performExportToHV(targetActor){try{let menuButtons={};let exportNewPC=false;let exportOverwritePC=false;let vaultInfo=false;let canOverwrite=false;let portrait,hvUID,portraitAddress,tokenAddress;hvUserToken=game.settings.get('herovaultfoundry','userToken');portrait="icons/svg/mystery-man.svg";if(targetActor.data.img!=undefined&&targetActor.data.token.img!=undefined){if(targetActor.data.img.includes("mystery-man")==-1)
...
if(hvDebug)
console.log("%cHeroVau.lt/Foundry Bridge | %cportrait includes: "+targetActor.data.img.includes("http"),hvColor1,hvColor4);if(targetActor.data.img.includes("http")==false){portraitAddress=game.data.addresses.remote+targetActor.data.img.trim();if(hvDebug){console.log("%cHeroVau.lt/Foundry Bridge | %c target: "+targetActor,hvColor1,hvColor4);console.log("%cHeroVau.lt/Foundry Bridge | %cportrait: "+portraitAddress,hvColor1,hvColor4);console.log("%cHeroVau.lt/Foundry Bridge | %csheet portrait: "+targetActor.data.img,hvColor1,hvColor4);}}
if(targetActor.data.token.img.includes("http")==false){tokenAddress=game.data.addresses.remote+targetActor.data.token.img.trim();await targetActor.update({'data.token.img':tokenAddress});if(hvDebug){console.log("%cHeroVau.lt/Foundry Bridge | %ctoken: "+tokenAddress,hvColor1,hvColor4);console.log("%cHeroVau.lt/Foundry Bridge | %csheet token: "+targetActor.data.token.img,hvColor1,hvColor4);}}}
if(hasProperty(targetActor,"data.flags.herovault.uid")){hvUID=targetActor.data.flags.herovault.uid;let accChk=await checkForAccess(hvUserToken,hvUID);canOverwrite=accChk.canAccess;}
vaultInfo=await getVaultSlots(hvUserToken);if(hvDebug)
console.log("%cHeroVau.lt/Foundry Bridge | %cvaultInfo: "+JSON.stringify(vaultInfo),hvColor1,hvColor4);let totalSlots=vaultInfo.totalSlots;let usedSlots=vaultInfo.usedSlots;let freeSlots=totalSlots-usedSlots;let bdy=`<div><p>You have ${freeSlots}/${totalSlots} character slots free.</p><div><hr/>`;if(hvDebug)
console.log("%cHeroVau.lt/Foundry Bridge | %ccan access/overwrite?: "+canOverwrite,hvColor1,hvColor4);if(freeSlots<1&&canOverwrite==false){bdy=`<div><p>Unfortunately you do not have enough open slots in your <a href="https://herovau.lt">HeroVau.lt</a> to import this PC.<br>Please upgrade your account or delete a PC from your account to free up some space.</p><div><hr/>`;new Dialog({title:"Import to your HeroVau.lt",content:bdy,buttons:{yes:{icon:"<i class='fas fa-check'></i>",label:`Ok`}},default:"yes"}).render(true);}else{if(freeSlots>0){menuButtons={...menuButtons,exportNew:{icon:"<i class='fas fa-file-export'></i>",label:`Export to HeroVau.lt as New PC`,callback:()=>exportNewPC=true}}
...
bdy=bdy+`<div><p><img src="${portrait}"><br>Please choose an action to perform:</p><div><hr/>`;new Dialog({title:"Import to your HeroVau.lt",content:bdy,buttons:menuButtons,default:"exportNew",close:async(html)=>{if(exportNewPC){hvUID="";let exportStatus=await exportPCtoHV(targetActor,hvUserToken,hvUID,true,portraitAddress,tokenAddress);if(exportStatus.error==true){ui.notifications.error("Error exporting: "+exportStatus.message);}else{targetActor.update({'flags.herovault.uid':exportStatus.charhash});ui.notifications.info(exportStatus.message);}}else if(exportOverwritePC){if(hvDebug)
console.log("export overwrite PC");let exportStatus=await exportPCtoHV(targetActor,hvUserToken,hvUID,false,portraitAddress,tokenAddress);if(exportStatus.error==true){ui.notifications.error("Error exporting: "+exportStatus.message);}else{targetActor.update({'flags.herovault.uid':exportStatus.charhash});ui.notifications.info(exportStatus.message);}}}}).render(true);}}catch(e){console.log(e);}}
const checkForAccess=async(hvUserToken,hvUID)=>{return new Promise((resolve)=>{let error=false;var xmlhttp=new XMLHttpRequest();xmlhttp.onreadystatechange=function(){if(this.readyState==4&&this.status==200){let responseJSON=JSON.parse(this.responseText);if(hvDebug)
console.log("%cHeroVau.lt/Foundry Bridge | %c"+JSON.stringify(responseJSON),hvColor1,hvColor4);if(error){resolve(false);}else{resolve(responseJSON);}}};if(hvDebug){console.log("%cHeroVau.lt/Foundry Bridge | %cChecking if this account can access: "+hvUID,hvColor1,hvColor4);console.log("%cHeroVau.lt/Foundry Bridge | %chttps://herovau.lt/foundrymodule.php?action=checkCharacter&userToken="+hvUserToken+"&charUID="+hvUID,hvColor1,hvColor4);}
xmlhttp.open("GET",heroVaultURL+"/foundrymodule.php?action=checkCharacter&userToken="+hvUserToken+"&charUID="+hvUID,true);xmlhttp.send();});};const getVaultSlots=async(hvUserToken)=>{return new Promise((resolve)=>{let error=false;var xmlhttp=new XMLHttpRequest();xmlhttp.onreadystatechange=function(){if(this.readyState==4&&this.status==200){let responseJSON=JSON.parse(this.responseText);if(hvDebug)
...
console.log("%cHeroVau.lt/Foundry Bridge | %chttps://herovau.lt/foundrymodule.php?action=getVaultSlots&userToken="+hvUserToken,hvColor1,hvColor4);xmlhttp.open("GET",heroVaultURL+"/foundrymodule.php?action=getVaultSlots&userToken="+hvUserToken,true);xmlhttp.send();});};const exportPCtoHV=(targetActor,userToken,charUID,importAsNew,portraitAddress,tokenAddress)=>{return new Promise((resolve)=>{let error=false;let action='';if(importAsNew)
action='importNewPC';else
action='importExistingPC';const gameSystem=game.data.system.id;let pcEncodedJSON=encodeURIComponent(JSON.stringify(targetActor.data));var xmlhttp=new XMLHttpRequest();xmlhttp.onreadystatechange=function(){if(this.readyState==4&&this.status==200){let responseJSON=JSON.parse(this.responseText);console.log(responseJSON)
if(hvDebug)
...
importCharacter(targetActor,responseJSON.downloadURL);}else{ui.notifications.warn("Unable find character.  Please contact HeroVau.lt support.");return;}}}};if(hvDebug)
console.log("%cHeroVau.lt/Foundry Bridge | %ccharUID: "+charUID,hvColor1,hvColor4)
xmlhttp.open("GET",heroVaultURL+"/foundrymodule.php?action=getCharacter&charUID="+encodeURIComponent(charUID),true);xmlhttp.send();}
async function importCharacter(targetActor,charURL){let error=false;var importPCID,errMsg,charDataStr,charImport;var xmlhttp=new XMLHttpRequest();xmlhttp.onreadystatechange=function(){if(this.readyState==4&&this.status==200){let responseJSON=JSON.parse(this.responseText);if(hvDebug)
...
if(hvDebug)
console.log("%cHLO Importer | %c Importing "+charImport.name,hvColor1,hvColor4);targetActor.importFromJSON(JSON.stringify(charImport));var request=new XMLHttpRequest();request.open('GET',charImport.token.img,true);request.onreadystatechange=function(){if(this.status===404){targetActor.update({'data.token.img':"icons/svg/mystery-man.svg"});}}
request.send();var request2=new XMLHttpRequest();request2.open('GET',charImport.img,true);request2.onreadystatechange=function(){if(this.status===404){if(hvDebug)
console.log("got a 404");targetActor.update({'img':"icons/svg/mystery-man.svg"});}}
request2.send();}}};xmlhttp.open("GET",charURL,true);xmlhttp.send();}
var Cookie={set:function(name,value,days){var domain,domainParts,date,expires,host;if(days){date=new Date();date.setTime(date.getTime()+(days*86400000));expires="; expires="+date.toGMTString();}else{expires="";}
host=location.host;if(host.split('.').length===1){document.cookie=name+"="+value+expires+"; path=/; SameSite=Strict";}else{domainParts=host.split('.');domainParts.shift();domain='.'+domainParts.join('.');if(host.includes("forge-vtt.com")){document.cookie=name+"="+value+expires+"; path=/; domain=.forge-vtt.com; SameSite=Strict";}else{document.cookie=name+"="+value+expires+"; path=/; domain="+domain+"; SameSite=Strict";document.cookie=name+"="+value+expires+"; path=/; domain="+host+"; SameSite=Strict";}
if(Cookie.get(name)==null||Cookie.get(name)!=value){domain='.'+host;document.cookie=name+"="+value+expires+"; path=/; domain="+domain+"; SameSite=Strict";}}},get:function(name){var nameEQ=name+"=";var ca=document.cookie.split(';');for(var i=0;i<ca.length;i++){var c=ca[i];while(c.charAt(0)==' '){c=c.substring(1,c.length);}
if(c.indexOf(nameEQ)==0)return c.substring(nameEQ.length,c.length);}
return null;},erase:function(name){Cookie.set(name,'',-1);}};let HVversionCompare=function(left,right){if(typeof left+typeof right!='stringstring')
...
return 0;}
function getHash(encodedHeroJSON){var xmlhttp=new XMLHttpRequest();xmlhttp.onreadystatechange=function(){if(this.readyState==4&&this.status==200){let responseJSON=JSON.parse(this.responseText);if(hvDebug)
console.log("%cHeroVau.lt/Foundry Bridge | %c"+JSON.stringify(responseJSON),hvColor1,hvColor4);let newHash=responseJSON.newHash;return newHash;}};xmlhttp.open("POST",heroVaultURL+"/foundrymodule.php",true);xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");xmlhttp.send('action=genHash&encodedChar='+encodedHeroJSON);}
export function exportToHVFromPBHLO(heroJSON,tAct){let error=false;let action='importNewPCFromPBHLO';var xmlhttp=new XMLHttpRequest();const gameSystem=game.data.system.id;let pcEncodedJSON=encodeURIComponent(heroJSON);let newHash=getHash(pcEncodedJSON);xmlhttp.onreadystatechange=function(){if(this.readyState==4&&this.status==200){let responseJSON=JSON.parse(this.responseText);if(responseJSON.error==false){ui.notifications.info("Successfully exported to HeroVau.lt.");tAct.update({'flags.herovault.uid':responseJSON.charhash});return responseJSON.charhash;}else{ui.notifications.warn("Unable to export to HeroVau.lt. Please try manually.");}}};xmlhttp.open("POST",heroVaultURL+"/foundrymodule.php",true);xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");xmlhttp.send('action='+action+'&userToken='+hvUserToken+'&encodedChar='+pcEncodedJSON+'&gamesystem='+gameSystem+"&charUID="+newHash);}
export function supportCheck(){if(hvUserToken){return true;}}
Hooks.on('init',()=>{game.modules.get('herovaultfoundry').api={exportToHVFromPBHLO:exportToHVFromPBHLO,supportCheck:supportCheck};Hooks.callAll('herovaultfoundryReady',game.modules.get('herovaultfoundry').api);});